...
- **PK 선정의 중요성**
  ...
  - 선택을 해야하는 상황입니다. 이 부분에 대한 정답은 없다고 판단했으며 선택은 아래와 같은 2가지로 구분할 수 있습니다.
    - 기본키의 변경 가능성(유연성)을 염두에 두고 PK의 장점을 이용하지 못하더라도, 인조 키를 그대로 사용한다.
    - 성능을 더 중요시한다면 PK를 유의미한 컬럼으로 설정하여 활용한다.
